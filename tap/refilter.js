  ...
  request.post({'url':lib.db+'/_bulk_docs', 'json':{'docs':docs}}, function(er, res) {
    ...
    request({'url':feed, 'json':true}, function(er, res) {
      ...
      request({'url':feed, 'json':true}, function(er, res) {
        ...
        request({'url':feed, 'json':true}, function(er, res) {
          if(er) throw er
          t.equal(res.body.results.length, 1000, 'Got 1000 documents through the filter')
